...
class StreetFighterCustomWrapper(gym.Wrapper):
    ...
    def _stack_observation(self):
        ...
        return np.stack([self.frame_stack[i][:, :, 0] for i in range(3)], axis=-1)
        
        # return self.frame_stack

    def preprocess(self, observation): 
        obs = downsample(observation, rate=2)
        # Grayscaling 
        gray = cv2.cvtColor(obs, cv2.COLOR_BGR2GRAY)
        # print(gray.shape)
        # Add the channels value
        channels = np.reshape(gray, (100,128,1))
        return channels 
        # return gray
    ...
    def reset(self):
        ...
        for _ in range(self.num_frames):
            obs, _reward, _done, info  = self.env.step(no_action)
        ...
        # diff_obs = self.diff_with_prev_obs(self.prev_obs)
        # self.frame_stack.append(diff_obs)
    ...
    def step(self, action):
        ...
        if not self.reset_round and self.stages >= NUM_STAGES:
            ...
            # return self.diff_with_prev_obs(self.prev_obs), 0, True, info
        ...
        # return diff_obs, custom_reward, custom_done, info # reward normalization
